...
function showUpdates(newOnly) {
    ...
    let updates;
    if (newOnly) {
        updates = myUpdates.newStories().sort((a, b) => {
            return b.ID - a.ID;
        });
    }
    else {
        updates = myUpdates.allUpdates.sort((a, b) => {
            return b.ID - a.ID;
        });
    }
...
function appendArticle(node, headlineText, bodyText, url, newUpdate) {
